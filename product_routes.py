...
# Define the blueprint for the product routes
...
# --------------------
# R (Read) - GET MASTER LIST & FILTERED REPORTS (Member 1 & 3 Logic)
# --------------------
...
def get_products():
    ...
    # Member 3's Logic: Check for a filter query parameter
    low_stock_filter = request.args.get('filter')
    
    base_sql = "SELECT * FROM Products"
    where_clause = ""
    
    # If the URL is /api/products?filter=low_stock
    if low_stock_filter == 'low_stock':
        # Default low stock threshold is set to 10
...
# --------------------
# C (Create) - POST NEW PRODUCT (Member 1's Form 1 Logic)
# --------------------
...
def add_product():
    ...
    try:
        ...
        # Check for specific MySQL errors (e.g., duplicate SKU, invalid FK)
...
# --------------------
# U (Update) - PUT PRODUCT STATUS/DETAILS (Member 1 & 4 Logic)
# --------------------
...
def update_product_details(product_id):
    ...
    try:
        ...
        # Member 1's Logic: Status Update
        if 'status' in data:
            fields_to_update.append("status = %s")
            values.append(data['status'])
        
        # Member 4's Logic: Price/Description Update (Also used by Member 3 for quantity updates)
        if 'price' in data:
...
# --------------------
# D (Delete) - DELETE PRODUCT (Member 4's Logic)
# --------------------
